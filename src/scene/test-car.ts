...
export class TestCar implements Scene, SteeringControl {
    ...
    updateSteering(input: JoystickInput): void {
        ...
        const steering = input.direction.x;
        const acceleration = -input.direction.y;
    ...
    private applySteering(steering: number): void {
        this.state.rotation += steering * this.state.steeringSpeed;
    }

    private applyAcceleration(acceleration: number): void {
        this.state.speed += acceleration * this.state.acceleration;
